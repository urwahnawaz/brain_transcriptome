...
```{r}
form.ram <- ~ Age + (1|StructureAcronym) + (1|Sex) + (1|Diagnosis) + PMI + pH + TotalNReads + MappingRate + (1|Region)
...
md.hdbr %<>% 
  ...
  mutate(Karyotype = toupper(Karyotype)) %>% 
  mutate(Sex = ifelse(str_detect(Karyotype,"XX"), "F",
                      ifelse(str_detect(Karyotype, "XY"), "M", "Unknown" )), 
         Diagnosis = c("Control"))
...
```{r}
form.hdbr <- ~ (1|Age) + (1|StructureAcronym) + (1|Sex) + (1|Hemisphere) + TotalNReads + MappingRate + (1|Regions)
